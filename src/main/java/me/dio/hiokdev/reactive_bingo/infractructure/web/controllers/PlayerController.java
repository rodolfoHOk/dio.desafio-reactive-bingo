...
public class PlayerController {
    ...
    private Mono<PlayerResponse> create(@RequestBody @Valid final PlayerRequest request) {
        return playerUseCases.create(request);
    }

    @PutMapping(value = "{id}", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    private Mono<PlayerResponse> update(
    ...
    private Mono<Void> delete(@PathVariable @Valid @MongoId(message = "{playerController.id}") final String id) {
        return playerUseCases.delete(id);
    }

    @GetMapping(value = "{id}", produces = MediaType.APPLICATION_JSON_VALUE)
    private Mono<PlayerResponse> findById(@PathVariable @Valid @MongoId(message = "{playerController.id}") final String id) {
        return playerUseCases.findById(id);
    }

    @GetMapping(produces = MediaType.APPLICATION_JSON_VALUE)
    private Mono<PagedPlayersResponse> findOnDemand(@Valid final PageablePlayersRequest request) {
